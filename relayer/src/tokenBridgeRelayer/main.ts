...
} from "@deltaswapio/deltaswap-sdk/lib/cjs/ethers-contracts";
import { TypedEvent } from "@deltaswapio/deltaswap-sdk/lib/cjs/ethers-contracts/common";
...
function handleRelayerEvent(
  ...
  (async () => {
    try {
      ...
      // now fetch and parse the deltaswap payload
      console.log(
...
function subscribeToEvents(
  ...
  deltaswap.off(
    deltaswap.filters.LogMessagePublished(sender),
    handleRelayerEvent
  );
  deltaswap.on(deltaswap.filters.LogMessagePublished(sender), handleRelayerEvent);
  console.log(
    `Subscribed to: ${chainName}, core contract: ${deltaswap.address}`
  );
}
...
async function main() {
  ...
  for (const chainId of SUPPORTED_CHAINS) {
    try {
      subscribeToEvents(
        deltaswapContract(CONFIG[chainId.toString()].deltaswap, SIGNERS[chainId]),
        chainId
      );
    } catch (e: any) {
      console.log(e);
